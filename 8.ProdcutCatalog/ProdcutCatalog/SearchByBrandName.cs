...
{
    internal class SearchByBrandName
    {
        public SearchByBrandName()
        {
            SearchBrand();
        }
        ...
        public void SearchBrand()
        {
            Console.Clear();
            char line = '_';
            Console.Write("Search By BrandName :");
            string search = Console.ReadLine().ToLower().Trim();
            foreach (var p in DemoProducts.viewProductsArr.Where(x => x.BrandName.Contains(search)))
            {
                for (int m = 0; m <= 45; m++)
                {
                    Console.Write(line);
