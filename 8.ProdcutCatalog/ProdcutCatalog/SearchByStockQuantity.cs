...
{
    public class SearchByStockQuantity
    {

        public SearchByStockQuantity()
        {
            SearchProp();
        }
        ...
        public void SearchProp()
        {
            Console.Clear();
            char line = '_';
            Console.Write("Search By Stock Quantity (High/Medium/Low):");
            string search = Console.ReadLine().ToLower().Trim();
            foreach (var p in DemoProducts.viewProductsArr.Where(x => x.StockStatus.Contains(search)))
            {
                for (int m = 0; m <= 45; m++)
                {
                    Console.Write(line);
